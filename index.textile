...
bq=.:http://en.wikiquote.org/wiki/Stan_Lee "With great power comes great responsibility." -Stan Lee

h1. Advanced Usages

p. Seriously, you should probably skim the "git community book":http://book.git-scm.com/ if you want to use the more advanced features.

h1. Advanced Usages

h2. Feature branches - Idea
...
h2. Feature branches - Benefits

# @master@ branch remains pristine locally.
# Can checkin frequently even if just a wip without breaking the build.
...
h2. Feature branches - Downsides

# More complex (but still possible) to maintain linear history in @master@
# Promotes longer development 'in a vacuum'

h1. Advanced Usages

h2. Feature branches - Commands

- create a branch := @git branch <branchname>@
- switch into branch := @git checkout <branchname>@
- create and switch into branch := @git checkout -b <branchname>@
- pull in new changes from master := @git rebase master <branchname>@
- merge changes back to master := @git checkout master; git merge --ff-only <branchname>@

h1. Advanced Usages

h2. Interactive Rebase - Idea

p. You may occasionally accumulate a small number of commits locally before you share them.  Interactive rebasing allows you the oppurtunity to clean up the commits in a number of ways before you push them up stream.


h1. Advanced Usages

h2. Interactive Rebase - Benefits
...
h2. Interactive Rebase - Example

p=. !{height:60%;}images/terminal_rebase_1.png!

h1. Advanced Usages

h2. Interactive Rebase - Example

p=. !{height:60%;}images/terminal_rebase_2.png!
